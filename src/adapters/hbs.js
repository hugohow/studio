// Adaptateur Studio HBS — via QuickStudio (Ruby on Rails, planning rendu en HTML).
// Pas d'API JSON : on parse la page planning. 1 requête = 1 jour pour les 8 salles.
//   GET /fr/studios/studio-hbs/bookings?date=YYYY-MM-DD
// Chaque salle : <div class="room" data-room="ID"> ... <span class="cell-N available|unavailable|closed"
//   data-start="<unix>" data-end="<unix>"> ... — les `available` sont les intervalles libres.
// ⚠️ Scraping HTML : plus fragile qu'une API si QuickStudio change son markup. Lecture seule.

const BASE = "https://www.quickstudio.com/fr/studios/studio-hbs/bookings";
...
// renvoie une valeur fixe (≈ placeholder) et non la vraie dispo -> on coupe à 14 jours.
const MAX_DAYS = 14;
...
function parseRoomNames(html) {
  const names = new Map();
  for (const m of html.matchAll(/<div class="room-box">([\s\S]*?data-room="(\d+)"[\s\S]*?)<div class="hours"/g)) {
    const id = m[2];
    const txt = m[1]
      .replace(/<[^>]+>/g, " ")
      .replace(/&nbsp;/g, " ")
      .replace(/\s+/g, " ")
      .trim();
    const name = (txt.split(/ {2,}| /)[0] || txt).split(/Studio de|Box solo|Mini Studio -/)[0].trim();
